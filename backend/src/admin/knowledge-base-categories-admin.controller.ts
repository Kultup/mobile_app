...
import { KnowledgeBaseCategory } from '../knowledge-base/schemas/knowledge-base-category.schema';
import { KnowledgeBaseArticle } from '../knowledge-base/schemas/knowledge-base-article.schema';
import { JwtAuthGuard } from '../common/guards/jwt-auth.guard';
import { RolesGuard } from '../common/guards/roles.guard';
import { Roles } from '../common/decorators/roles.decorator';

@Controller('admin/knowledge-base/categories')
@UseGuards(JwtAuthGuard, RolesGuard)
@Roles('super_admin', 'training_admin')
export class KnowledgeBaseCategoriesAdminController {
  constructor(
    @InjectModel(KnowledgeBaseCategory.name) private categoryModel: Model<KnowledgeBaseCategory>,
  ...
  async delete(@Param('id') id: string) {
    // Перевірити чи є статті в цій категорії
    const articlesCount = await this.articleModel.countDocuments({ category_id: id }).exec();
    
    if (articlesCount > 0) {
      throw new BadRequestException(`Cannot delete category: ${articlesCount} article(s) are using this category`);
    }
