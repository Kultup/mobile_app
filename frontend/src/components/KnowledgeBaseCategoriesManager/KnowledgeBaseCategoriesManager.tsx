...
const KnowledgeBaseCategoriesManager = () => {
  ...
  const createMutation = useMutation({
    mutationFn: (category: CreateCategoryDto) => categoriesService.createKnowledgeBaseCategory(category),
    onSuccess: () => {
  ...
  if (error) {
    return (
      <Box p={3}>
        <Alert severity="error">Помилка завантаження категорій бази знань</Alert>
      </Box>
    );
  }
  ...
  return (
    <Box>
      <Box display="flex" justifyContent="space-between" alignItems="center" mb={2}>
        <Typography variant="h6">Категорії бази знань</Typography>
        <Button
          variant="contained"
          size="small"
          startIcon={<AddIcon />}
      ...
      <Dialog open={deleteDialogOpen} onClose={() => setDeleteDialogOpen(false)}>
        ...
        <DialogContent>
          <Typography>
            Ви впевнені, що хочете видалити цю категорію? Якщо в ній є статті, видалення неможливе.
          </Typography>
        </DialogContent>
        <DialogActions>
...
export default KnowledgeBaseCategoriesManager;
